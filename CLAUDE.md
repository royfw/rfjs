...
- **Publish (GitHub Actions)**: `cd-publish-npmjs.yml` is a manual `workflow_dispatch` (Actions tab → Run workflow). It checks out the versioned branch (default `release/stable`) and runs plain `changeset publish` — which publishes every public package whose version isn't yet on npm and skips private ones. (GitLab's toolkit publish guards on `changeset status`, which wrongly reports "no releases" once versioning has consumed the changesets — so publish lives on GitHub instead. To hold a package back from publish, set `"private": true` — the only thing `changeset publish` honors.)
...
- Release workflow: `pnpm changeset:add` → commit to `main` → PR `main → release/*` and merge (GitHub Actions versions, opens a PR back to `main`) → run the `cd-publish-npmjs.yml` workflow against `release/stable` (GitHub Actions publishes to npm)
- There is **no `publish/npmjs` branch** — it was retired 2026-08-27. It existed only to hold the versioned state for the publish job, but the workflow takes the branch as a `ref` input, so `release/stable` (which already carries the bumps) serves directly. One less long-lived branch is one less source of drift: it had accumulated five phantom commits and lost its ancestry with `main` entirely. The commit it last published from stays reachable through that release's ten package tags.
- **Merge the release round-trip PRs with "Create a merge commit", never "Squash and merge".** Feature PRs into `main` are squashed — that is the convention and it should stay. But the release branches round-trip the *same* commits, and a squash mints a new single-parent commit with no link back to its source, so git stops seeing the branches as related: `main` and `release/stable` each report the other as "behind" while their content is byte-identical, and every release adds two more phantom commits. (The retired `publish/npmjs` reached five.) GitHub cannot pick a merge method per target branch, so this is a convention, not a setting — the button defaults to whatever you used last.
