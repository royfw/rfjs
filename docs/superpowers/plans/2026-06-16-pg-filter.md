...
describe('buildPgWhere', () => {
  ...
  it('mixes column + jsonb leaves with contiguous params across the boundary', () => {
    const group: PgFilterGroup = {
      logic: 'and',
      filters: [
    ...
    const { where, values } = buildPgWhere(config, group);
    // values appear in $-order: column 'cust' ($1), jsonb 80 ($2), column date ($3)
    expect(values).toEqual(['cust', 80, '2026-01-01']);
    expect(where).toContain('$1');
    expect(where).toContain('$2');
    expect(where).toContain('$3');
    expect(where).not.toContain('$4');
  });
...
describe('buildPgFilter', () => {
  ...
  it('threads params WHERE→ORDER BY and splits values vs countValues', () => {
    ...
    // WHERE has 1 param ($1='cust'); ORDER BY jsonb path param is $2
    expect(res.countValues).toEqual(['cust']);
    expect(res.values).toEqual(['cust', 'score']);
...
> 註:jsonb ORDER BY 的 path 參數值是欄位路徑字串(`buildJsonbOrderBy` 以 `#>> $n` 取值),此處 `'score'` 即該 path 參數。
