...
export interface UseAiAssist {
  ...
  runStream<T>(req: Omit<CompleteRequest, 'signal'>, parse: (raw: string) => T): Promise<T | null>;
}
...
export function useAiAssist(): UseAiAssist {
  ...
  const runStream = React.useCallback(async <T,>(req: Omit<CompleteRequest, 'signal'>, parse: (raw: string) => T): Promise<T | null> => {
    const settings = loadAiSettings();
    if (!isConfigured(settings)) {
      setError(new AiError('config', 'AI connection is not configured'));
      return null;
    ...
    let reasoning = '';
    try {
      const raw = await createAiClient(settings).stream({ ...req, signal: ctl.signal }, (d) => {
  ...
  return { ready, loading, error, streamText, streamReasoning, cancel, run, runStream };
}
